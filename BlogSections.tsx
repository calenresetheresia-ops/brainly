...
export const BlogHero: React.FC<SectionProps> = ({ onBlogClick }) => (
  <section className="bg-[#EBF5FF] pt-32 pb-24">
    <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
      <div>
        ...
        <h1 
          className="text-4xl lg:text-5xl font-extrabold text-[#2F327D] leading-tight mb-8 cursor-pointer hover:text-primary transition-colors"
          onClick={onBlogClick}
        >
        ...
        <button 
          onClick={onBlogClick}
          className="px-10 py-4 bg-[#49BBBD] text-white rounded-xl font-bold shadow-lg shadow-[#49BBBD]/20 hover:scale-105 active:scale-95 transition-all"
        >
          Start learning now
        </button>
      </div>
      <div className="relative cursor-pointer" onClick={onBlogClick}>
        <div className="rounded-3xl overflow-hidden shadow-2xl relative z-10 border-[10px] border-white/20">
          <img src="https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&q=80&w=800" alt="SwiftUI" className="w-full aspect-[4/3] object-cover" />
        </div>
      </div>
    </div>
  </section>
...
export const RelatedBlogList: React.FC<SectionProps> = ({ onBlogClick }) => (
      ...
      <div className="grid md:grid-cols-2 gap-10 mb-12">
        {RELATED_BLOGS.map(blog => (
          <div key={blog.id} className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-50 group hover:shadow-xl transition-all">
            ...
            <h3 
              className="text-2xl font-bold text-[#2F327D] mb-6 leading-tight group-hover:text-primary transition-colors cursor-pointer"
              onClick={onBlogClick}
            >
              {blog.title}
            </h3>
            ...
            <div className="flex items-center justify-between pt-8 border-t border-slate-50">
              <button 
                onClick={onBlogClick}
                className="text-slate-400 font-bold text-sm underline underline-offset-8 decoration-2 hover:text-primary hover:decoration-primary transition-all"
              >
                Read more
              </button>
...
export const MarketingArticleList: React.FC<SectionProps> = ({ onBlogClick }) => (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      {MARKETING_ARTICLES.map(art => (
        <div key={art.id} className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-100 hover:shadow-2xl transition-all group">
          <div className="aspect-[4/3] rounded-2xl overflow-hidden mb-6 relative cursor-pointer" onClick={onBlogClick}>
          ...
          <h4 
            className="text-xl font-bold text-[#2F327D] mb-4 leading-tight group-hover:text-primary transition-colors cursor-pointer"
            onClick={onBlogClick}
          >
            {art.title}
          </h4>
